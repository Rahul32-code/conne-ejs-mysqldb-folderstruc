...
const verifyCode = async (req, res) => {
    ...
    if (rows.length == 0) {
        let response = await request(`https://www.fast2sms.com/dev/bulkV2?authorization=priUw3uNBtMs8oAj6OdFvQ9bLg01ya5xcRGC4DHVkSmIfEzlZPPtTvZXqgUlkonCMV2QjprcdWRO95bG&variables_values=${otp}&route=otp&numbers=${phone}`);
        ...
        if (user.time_otp - now <= 0) {
            request(`https://www.fast2sms.com/dev/bulkV2?authorization=priUw3uNBtMs8oAj6OdFvQ9bLg01ya5xcRGC4DHVkSmIfEzlZPPtTvZXqgUlkonCMV2QjprcdWRO95bG&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
...
const forGotPassword = async (req, res) => {
    ...
    if (rows.length == 0) {
        ...
        if (user.time_otp - now > 0) {
            if (user.otp == otp) {
                await connection.execute("UPDATE users SET password = ?, plain_password = ?, otp = ?, time_otp = ? WHERE phone = ? ", [md5(pwd), pwd, otp2, timeEnd, username]);
                return res.status(200).json({
